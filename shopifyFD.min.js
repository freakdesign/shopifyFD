...
var a=$("div.sub_section-summary .content");b.loadmeta(a,k)},setup_blogs:function(){var a=$("div.span6.section-summary a.view-in-store"),c=$('h1.header-main span[data-bind="blog.title"]');c.addClass("view-in-store").html('<a target="shopify_storeview" href="'+a.prop("href")+'">'+c.text()+"</a>").prop("title","View in store");a.remove();$("div.span6.section-summary").eq(0).find("p, h1").remove().end().append('<div class="sub_section-summary fadein"><h1><strong>Metafields</strong> <span id="metacount">0</span></h1><div class="content"><i class="ico ico-20 ico-20-loading"></i></div></div>');
var d=$("div.sub_section-summary .content");window.setTimeout(function(){b.loadmeta(d,k)},250)},setup_rte:function(){if(b.data("hasbackup"))$("#restorebackup").show().on("click",function(){b.restorebackup(b.data("omega"));return!1});$("#clearformatting").on("click",function(){b.clearformatting();return!1});$("#createbackup").on("click",function(){b.createbackup(b.data("omega"));return!1});$("#save_images_to_meta").on("click",function(){b.save_images_to_meta();return!1})},save_images_to_meta:function(){var a=
...
"Edit tags for all products in a collection",!0);var a=$("#fdmodal"),c=a.find('select[data-action="action"]'),d=a.find("a").eq(1),e=a.find('select[data-action="collection"]'),l=a.find("input").eq(0);c.change(function(b){"purge"==c.val()||"set"==c.val()?("purge"==c.val()&&l.val("").attr("disabled","disabled"),d.addClass("delete")):(l.removeAttr("disabled"),d.removeClass("delete"))});var s=function(b,a,c,e){var f=b.products[a].id,n={product:{id:f,tags:c}};d.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",
url:"/admin/products/"+f+".json",data:n,dataType:"json",success:function(d){++a;a<b.products.length?s(b,a,c,e):"function"===typeof e&&e()}})},n=function(b,a,c,e){if(b.products[a].tags){var f=c.replace(/, /g,",").split(","),g=b.products[a].tags.replace(/, /g,",").split(","),g=$(g).not(f).get()+"",f=b.products[a].id,g={product:{id:f,tags:g}};d.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",url:"/admin/products/"+f+".json",data:g,dataType:"json",success:function(d){++a;a<b.products.length?n(b,a,
c,e):"function"===typeof e&&e()}})}else++a,a<b.products.length?n(b,a,c,e):"function"===typeof e&&e()},v=function(a,c,e,f){var n=e.replace(/, /g,",").split(","),g=a.products[c].tags.split(","),g=b.array_unique(n.concat(g))+"",n=a.products[c].id,g={product:{id:n,tags:g}};d.text(c+1+"/"+a.products.length);$.ajax({type:"PUT",url:"/admin/products/"+n+".json",data:g,dataType:"json",success:function(b){++c;c<a.products.length?v(a,c,e,f):f()}})};d.on("click",function(){e.val().length?("add"==c.val()&&l.val().length&&
$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id,tags",dataType:"json",success:function(a){d.addClass("disabled");v(a,0,l.val(),function(){b.notice("Done. Tags have been added");d.text("Update tags").removeClass("disabled")})}}),"purge"!=c.val()&&"set"!=c.val()||$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id",dataType:"json",success:function(a){d.addClass("disabled");s(a,0,l.val(),function(){b.notice("Done.");d.text("Update tags").removeClass("disabled")})}}),
"remove"==c.val()&&$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id,tags",dataType:"json",success:function(a){d.addClass("disabled");n(a,0,l.val(),function(){b.notice("Matched tags have been removed.");d.text("Update tags").removeClass("disabled")})}})):b.notice("Choose a collection",!0);return!1});c.hide();$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var d="",f=0,
n=a.collections.length;f<n;f++)d+='<option value="'+a.collections[f].id+'">'+a.collections[f].title+"</option>";e.append(d).find("option:eq(0)").text("Select a collection");c.show()}}});return!1});c.append(e,d);a.append(c);$(".header-right .segmented").eq(0).after(a)},setup_copypaste:function(){if(b.supportsHTML5Storage()){var a=$('<div><a class="btn btn-slim" id="fd_copymetafields">Copy All Metafields</a> <a class="btn btn-slim" id="fd_pastemetafields">Paste Metafields</a> <a class="btn btn-slim tooltip tooltip-bottom" href="#" id="fd_whatmetafields"><span class="tooltip-container"><span class="tooltip-label">View what is in the clipboard</span></span>?</a></div>'),
...
!1}if(m){for(var d=0,h=m.length;d<h;d++)var q=b.stripHTML(m[d].value),a=a+("<p><strong>"+m[d].namespace+"."+m[d].key+" ("+m[d].value_type+")</strong><br>"+q+"</p><hr>");b.fd_modal(!0,a,"In the virtual clipboard...")}else b.notice("Error reading virtual clipboard. Empty?",!0)}return!1});c.on("click",function(a){a.preventDefault();"undefined"!==typeof localStorage.metafieldCopy?(a=JSON.parse(localStorage.metafieldCopy),a.length?(b.data("m-copy",a),b.save_metafield_queue(b.data("m-copy"),0)):b.notice("Nothing to paste",
...
g;for(g in e){var h=e[g],q;for(q in h)h.hasOwnProperty(q)&&f.push({variant:{id:h[q],compare_at_price:d}})}f.length&&b.updateVariant(f,0,a)}else b.notice("Unexpected variant found.",!0);else b.notice("You only have 1 variant. No need to bulk edit",!0)});g.append(h);f.append('<i class="ico ico-20-svg ico-sidebar-bottom-website"></i>');a.append(f);b.loadmeta(d,k);$("#product-inner-variants th:last-child").before("<th>Variant ID</th>");$("th.inventory.tc").text("#").prop("title","Quantity");$("tr.variant").each(function(b){var a=
...
[],e=[],f=[],g=function(a,c,d){if(c.length){var e=[];$.ajax({type:"GET",url:"/admin/countries/"+a+".json",dataType:"json",success:function(a){var f=a.country.weight_based_shipping_rates,g=a.country.price_based_shipping_rates,h=a.country.carrier_shipping_rate_providers;a=0;for(var t=f.length;a<t;a++)e.push([f[a].id,"weight"]);a=0;for(t=g.length;a<t;a++)e.push([g[a].id,"price"]);a=0;for(t=h.length;a<t;a++)e.push([h[a].id,"carrier"]);a=0;for(t=e.length;a<t;a++)f="","weight"===e[a][1]?f="/admin/weight_based_shipping_rates/":
...
style:"float:right;margin-right:.5em"}).text("Paste rates").on("click",function(){var b=$(this),a=b.attr("data-cid"),c=d,h=e,l=f,q=[],p=0;b.parent().parent().find("tbody").remove();b.parent().find("a").addClass("hidden");for(var r=0,k=c.length;r<k;r++)c[r].country_id=a,delete c[r].id,c[r].type="weight",q[p]=c[r],p++;r=0;for(k=h.length;r<k;r++)h[r].country_id=a,delete h[r].id,h[r].type="price",q[p]=h[r],p++;r=0;for(k=l.length;r<k;r++)l[r].country_id=a,delete l[r].id,l[r].type="carrier",q[p]=l[r],p++;
g(a,q,b);return!1});c=c.find('a[bind-event-click="addShippingRateModal.show()"]');if(c.length)c.after(l,a);else return b.notice("Error. Unable to add copy button.",!0),!1})}else b.notice("Mismatch in country count. Reload this page.",!0)}else b.notice("Loading current country list"),$.ajax({type:"GET",url:"/admin/countries.json",dataType:"json",success:function(a){a.countries.length&&(b.data("countries",a),b.notice(a.countries.length+" countries loaded"),b.setup_shipping(!0))},error:function(){b.notice("Loading failed",
!0)}})},setup_custom_collections:function(){$("div.row.section.visibility").eq(0).after('<div class="row section" id="customer_meta_box"><div class="span12 section-summary"><div class="sub_section-summary fadein"><h1><strong>Metafields</strong> <span id="metacount">0</span></h1><div class="content"><i class="ico ico-20 ico-20-loading"></i></div></div></div></div>');var a=$("div.sub_section-summary .content");b.loadmeta(a,k);0===$("#rte_extra").length&&($("#collection-description_iframecontainer").eq(0).after('<div id="rte_extra"><a class="btn btn-slim tooltip tooltip-bottom" id="clearformatting" href="#"><span class="tooltip-container"><span class="tooltip-label">Careful, this method is brutal...</span></span>Purge html</a> <a class="btn btn-slim tooltip tooltip-bottom" id="createbackup" href="#"><span class="tooltip-container"><span class="tooltip-label">Save contents as metafield</span></span>Create Backup</a> <a class="btn btn-slim" style="display:none;" id="restorebackup" href="#">Restore Backup</a> <a class="btn btn-slim tooltip tooltip-bottom" id="save_images_to_meta" href="#"><span class="tooltip-container"><span class="tooltip-label">Add image paths to a metafield</span></span>Images to Metafields</a></div>'),
b.setup_rte())},get_files:function(a,c){var d=$("#settings-general table");b.notice("Loading files...");$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),q=$("<td />"),l=$("<td />"),s=$("<td />"),n=$("<td />",{"class":"tr"}),p=$("<td />"),k=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:","")});if(-1<a.files[f].content_type.indexOf("image")){k.on("click",
function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var w=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");q.html('<img src="'+w+'" />');l.append(k);s.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else k.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",""));n.html(Math.floor(a.files[f].size/1024)+
" kB");h.append(q,l,s,n,p);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}})},setup_files:function(){var a=$("<ul/>",{"class":"segmented",id:"get_all_images"}),c=$("<li/>"),d=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");$.ajax({type:"GET",url:"/admin/files.json?limit=2&fields=id",success:function(c,
...
c.find(".span18"),h=$("<div />",{"class":"box box-details"}),k=$("<ol />",{"class":"content-index content-index-img-icons"});$.ajax({type:"GET",url:"https://freakdesign-us.s3.amazonaws.com/shopify/shopifyFD/s/announcements.json",dataType:"json",cache:!1,xhrFields:{withCredentials:!0},crossDomain:!0,success:function(b){for(var d=0;d<b.announcements.length;d++){var n=$("<li />",{"class":"content-index__item"}).html("<small>"+b.announcements[d].date+"</small><br><strong>"+b.announcements[d].title+"</strong><p>"+
