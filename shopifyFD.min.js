(function(){if("undefined"!==typeof Shopify)if("undefined"!==typeof jQuery)if($("#shopifyjs").length)"function"===typeof Shopify.Flash.error&&Shopify.Flash.error("Error. ShopifyFD already loaded!");else{var l=document;if(1<l.URL.indexOf("myshopify.com/admin")){var v=$("<div />",{"class":"box error animated fadein"}).html('Warning - Some ShopifyFD features do not currently work on this view. Fixes are being looked at, but for now expect errors. See this <a target="_blank" href="https://ecommerce.shopify.com/c/shopify-apis-and-technology/t/dashboard-endpoints-failing-link-lists-shipping-files-etc-212161">related post</a>.'),
b=function(){var p={debug:!1,drag_on:!1,alpha:!1,omega:!1,countries:!1,wait:1E3,api_count:0,api_limit:250,metafields:{},autosave:!1,content:$("#content"),body:$("body")};return{escape:function(b){return b.replace(/[\\]/g,"\\\\").replace(/[\"]/g,'\\"').replace(/[\/]/g,"\\/").replace(/[\b]/g,"\\b").replace(/[\f]/g,"\\f").replace(/[\n]/g,"\\n").replace(/[\r]/g,"\\r").replace(/[\t]/g,"\\t")},redirect:function(b){if("undefined"!==typeof b&&"function"===typeof Turbolinks)try{Turbolinks.visit(b)}catch(c){return!1}},
...
function(){var a=$(this);if(b.data("products")){for(var c=b.data("products"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),f=0,k=c.products.length;f<k;f++)d+='<option value="'+c.products[f].handle+'">'+c.products[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",
...
d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),f=0,k=c.collections.length;f<k;f++)d+='<option value="'+c.collections[f].handle+'">'+c.collections[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=
...
1,title:b.pages[c].title,link_type:"page",subject_id:b.pages[c].id});g(a)},error:function(){b.notice("Error! Are you sure you have access to pages?",!0)}});return!1},e=function(){$.ajax({type:"GET",url:"/admin/products/vendors.json",dataType:"json",success:function(b){for(var a={utf8:"\u2713",_method:"create",link_list:{handle:"all-vendors",title:"All Vendors",links:[]}},c=0,d=b.vendors.length;c<d;++c)a.link_list.links.push({position:c+1,title:b.vendors[c],link_type:"http",subject:"/collections/vendors?q="+
encodeURIComponent(b.vendors[c].toLowerCase()).replace(/%20/g,"+")});g(a)}})},f=function(a,c){"undefined"!==typeof a&&$.ajax({type:"POST",url:"/admin/link_lists",data:a,success:function(a,c,d){(a=d.getResponseHeader("X-XHR-Redirected-To"))?(linklistID=href=a.split("/").pop(),isNaN(linklistID)?b.notice("Error creating linklist - ID not returned",!0):(b.notice("Link list added"),b.redirect("/admin/link_lists/"+linklistID))):b.notice("link list copied")},error:function(){b.notice("Error creating linklist",
!0)}})},g=function(a){$.ajax({type:"POST",url:"/admin/link_lists.json",data:JSON.stringify(a),contentType:"application/json;charset=utf-8",success:function(a){b.notice("Link list added");b.redirect("/admin/link_lists/"+a.link_list.id)},error:function(a){406===a.status?(b.notice("Linklist creation attempted. Reload page."),b.redirect("/admin/links")):b.notice("Error creating linklist",!0)}})},h=$("<a/>",{href:"#","class":"tooltip-bottom tooltip",style:"margin-right:1.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Make a copy of this linklist</span></span>Copy').on("click",
...
c,e):"function"===typeof e&&e()}})}else++a,a<b.products.length?w(b,a,c,e):"function"===typeof e&&e()},s=function(a,c,e,f){var g=e.replace(/, /g,",").split(","),h=a.products[c].tags.split(","),h=b.array_unique(g.concat(h))+"",g=a.products[c].id,h={product:{id:g,tags:h}};d.text(c+1+"/"+a.products.length);$.ajax({type:"PUT",url:"/admin/products/"+g+".json",data:h,dataType:"json",success:function(b){++c;c<a.products.length?s(a,c,e,f):f()}})};d.on("click",function(){e.val().length?("add"==c.val()&&f.val().length&&
...
d<e.length?f(d,e):(b.notice("Weight updated"),a.removeClass("is-loading").text("Save"))}})};a.addClass("is-loading").text("");f(0,e)}else b.notice("Did you set a weight?",!0);return!1});return!1});$("<span/>",{"class":"hidden"});a.append(r);$('tr.variant input[type="checkbox"]').on("change",function(){1<$('tr.variant input[type="checkbox"]:checked').length?r.removeClass("hidden"):r.addClass("hidden")})}else b.notice("ShopifyFD error : setup_products : unable to add bulk weight editor");d.length&&
"object"===typeof g&&1<Object.keys(g).length&&h.after(n)},setup_pages:function(){var a=$(".section.description .section-summary").eq(0);if(a.length){var c=a.find("a").eq(0).attr("href");a.after('<div class="sub_section-summary fadein"><h1>Metafields <span id="metacount" class="animated bounce">0</span></h1><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></div>').remove();a=$("div.metafield-content");b.loadmeta(a,p)}else b.notice("ShopifyFD error : setup_pages : Metafield target HTML not found",
...
b.data("countries").countries.length){var d=[],e=[],f=[],g=function(a,c,d){if(c.length){var e=[];$.ajax({type:"GET",url:"/admin/countries/"+a+".json",dataType:"json",success:function(a){var f=a.country.weight_based_shipping_rates,g=a.country.price_based_shipping_rates;a=a.country.carrier_shipping_rate_providers;for(var h=0,t=f.length;h<t;h++)e.push([f[h].id,"weight_based"]);h=0;for(t=g.length;h<t;h++)e.push([g[h].id,"price_based"]);h=0;for(t=a.length;h<t;h++)e.push([a[h].id,"carrier_based"]);h=0;
for(t=e.length;h<t;h++)$.ajax({type:"POST",url:"/admin/shipping_rates/"+e[h][0],data:{utf8:"\u2713",_method:"delete",shipping_rate:{type:e[h][1]}},success:function(b){},error:function(a){b.notice("Failed to replace rate",!0)}});b.save_new_rates_FIX(c,0,d)},fail:function(){b.notice("I failed to get the country.json",!0)}})}else b.notice("Missing items to add",!0)};$("div.shipping-rate-table").each(function(a){var c=$(this),n=$("<a/>",{href:"#","data-country":b.data("countries").countries[a].name,"data-cid":b.data("countries").countries[a].id,
...
style:"float:right;margin-right:.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Will replace ALL existing rates.</span></span>Paste rates (replace)').on("click",function(){var a=$(this),b=a.attr("data-cid"),c=d,h=e,k=f,n=[],l=0;a.parent().parent().find("tbody").remove();a.parent().find("a").addClass("hidden");for(var q=0,p=c.length;q<p;q++)c[q].country_id=b,delete c[q].id,c[q].type="weight",n[l]=c[q],l++;q=0;for(p=h.length;q<p;q++)h[q].country_id=b,delete h[q].id,h[q].type=
"price",n[l]=h[q],l++;q=0;for(p=k.length;q<p;q++)k[q].country_id=b,delete k[q].id,k[q].type="carrier",n[l]=k[q],l++;g(b,n,a);return!1});c=c.find('a[bind-event-click="addShippingRateModal.show()"]');if(c.length)c.after(n,a);else return b.notice("Error. Unable to add copy button.",!0),!1})}else b.notice("Mismatch in country count. Reload this page.",!0)}else b.notice("Loading current country list"),$.ajax({type:"GET",url:"/admin/countries.json",dataType:"json",success:function(a){a.countries.length&&
...
a=$("div.metafield-content"),b.loadmeta(a,p),0===$("#rte_extra").length&&($("#collection-description_iframecontainer").eq(0).after('<div id="rte_extra"><a class="btn btn-slim tooltip tooltip-bottom" id="clearformatting" href="#"><span class="tooltip-container"><span class="tooltip-label">Careful, this method is brutal...</span></span>Purge html</a> <a class="btn btn-slim tooltip tooltip-bottom" id="createbackup" href="#"><span class="tooltip-container"><span class="tooltip-label">Save contents as metafield</span></span>Create Backup</a> <a class="btn btn-slim" style="display:none;" id="restorebackup" href="#">Restore Backup</a> <a class="btn btn-slim tooltip tooltip-bottom" id="save_images_to_meta" href="#"><span class="tooltip-container"><span class="tooltip-label">Add image paths to a metafield</span></span>Images to Metafields</a></div>'),
b.setup_rte())):b.notice("ShopifyFD error : setup_custom_collections : target html not found",!0)},get_files:function(a,c){var d=$("#settings-general table");b.notice("Loading files...");$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),k=$("<td />"),n=$("<td />"),r=$("<td />"),l=$("<td />",{"class":"tr"}),p=$("<td />"),s=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:",
"")});if(-1<a.files[f].content_type.indexOf("image")){s.on("click",function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var v=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");k.html('<img src="'+v+'" />');n.append(s);r.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else s.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",
""));l.html(Math.floor(a.files[f].size/1024)+" kB");h.append(k,n,r,l,p);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}})},setup_files:function(){var a=$(".header-row .header-right").eq(0);if(a.length){var c=$("<ul/>",{"class":"segmented",id:"get_all_images"}),d=$("<li/>"),e=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");
...
typeof b)return p[a];p[a]=b},toggleStyle:function(){$("html").toggleClass("shopifyJSoverride")},isloading:function(){return b.data("content").hasClass("loading")}}}();b.load_css();b.init()}else alert("Error. This should be run within the Shopify Admin page.")}else alert("Error. jQuery not found.")})();