...
c=a.find("a.create"),d=a.find("a.createint");a.find("a.update");a.find("a.delete");var e=a.find("#bulk_namespace"),f=a.find("#bulk_key"),g=a.find("#bulk_value"),h=a.find("textarea.debug"),r=function(){return 1<e.val().length&&1<f.val().length&&1<g.val().length?!0:!1};c.off("click").on("click",function(){if(r){var a={metafield:{namespace:e.val(),key:f.val(),value:g.val(),value_type:"string"}};b.bulk_save_metafield_queue(a,0,h)}else b.notice("No empty fields allowed")});d.off("click").on("click",function(){if(r)if(isNaN(g.val()))b.notice("Value not an integer",
...
url:"/admin/products/"+f+".json",data:g,dataType:"json",success:function(d){++a;a<b.products.length?t(b,a,c,e):e()}})},p=function(b,a,c,e){if(b.products[a].tags){var f=c.replace(/, /g,",").split(","),g=b.products[a].tags.replace(/, /g,",").split(","),g=$(g).not(f).get()+"",f=b.products[a].id,g={product:{id:f,tags:g}};d.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",url:"/admin/products/"+f+".json",data:g,dataType:"json",success:function(d){++a;a<b.products.length?p(b,a,c,e):e()}})}else++a,a<b.products.length?
p(b,a,c,e):e()},v=function(a,c,e,f){var g=e.replace(/, /g,",").split(","),p=a.products[c].tags.split(","),p=b.array_unique(g.concat(p))+"",g=a.products[c].id,p={product:{id:g,tags:p}};d.text(c+1+"/"+a.products.length);$.ajax({type:"PUT",url:"/admin/products/"+g+".json",data:p,dataType:"json",success:function(b){++c;c<a.products.length?v(a,c,e,f):f()}})};d.on("click",function(){e.val().length?("add"==c.val()&&k.val().length&&$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id,tags",
...
e.val()+"&fields=id,tags",dataType:"json",success:function(a){d.addClass("disabled");p(a,0,k.val(),function(){b.notice("Matched tags have been removed.");d.text("Update tags").removeClass("disabled")})}})):b.notice("Choose a collection",!0);return!1});c.hide();$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var d="",f=0,p=a.collections.length;f<p;f++)d+='<option value="'+a.collections[f].id+'">'+a.collections[f].title+
...
g;for(g in e){var h=e[g],r;for(r in h)h.hasOwnProperty(r)&&f.push({variant:{id:h[r],compare_at_price:d}})}f.length&&b.updateVariant(f,0,a)}else b.notice("Unexpected variant found.",!0);else b.notice("You only have 1 variant. No need to bulk edit",!0)});g.append(h);f.append('<i class="ico ico-20-svg ico-sidebar-bottom-website"></i>');a.append(f);b.loadmeta(d,n);$("#product-inner-variants th:last-child").before("<th>Variant ID</th>");$("th.inventory.tc").text("#").prop("title","Quantity");$("tr.variant").each(function(b){var a=
...
b.setup_rte())},get_files:function(a,c){var d=$("#settings-general table");b.notice("Loading files...");$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),r=$("<td />"),k=$("<td />"),l=$("<td />"),p=$("<td />",{"class":"tr"}),n=$("<td />"),u=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:","")});if(-1<a.files[f].content_type.indexOf("image")){u.on("click",
function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var w=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");r.html('<img src="'+w+'" />');k.append(u);l.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else u.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",""));p.html(Math.floor(a.files[f].size/1024)+
" kB");h.append(r,k,l,p,n);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}})},setup_files:function(){var a=$("<ul/>",{"class":"segmented",id:"get_all_images"}),c=$("<li/>"),d=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");$.ajax({type:"GET",url:"/admin/files.json?limit=2&fields=id",success:function(c,
