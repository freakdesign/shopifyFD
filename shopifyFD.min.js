...
C=function(a){return a.replace(/[\\]/g,"\\\\").replace(/[\"]/g,'\\"').replace(/[\/]/g,"\\/").replace(/[\b]/g,"\\b").replace(/[\f]/g,"\\f").replace(/[\n]/g,"\\n").replace(/[\r]/g,"\\r").replace(/[\t]/g,"\\t")},r=function(a){if("undefined"!==typeof a&&"function"===typeof Turbolinks)try{Turbolinks.visit(a)}catch(b){return!1}},L=function(a,b){return a.getAttribute("value")?b.getAttribute("value")?a.innerHTML>b.innerHTML?1:-1:1:-1},f=function(a,b){"function"===typeof Shopify.Flash.error&&"function"===
...
!0);$("#metafield_value").val(b.value);$("#metafield_id").val(a.attr("data-id"));$("#shopifyjs_deletemetafield").removeClass("hide")}else Q()});$("#shopifyjs_deletemetafield").on("click",function(a){a.preventDefault();a=$("#metafield_id").val()||!1;if(!a)return f("Object ID Missing",!0),!1;a=z(a);-1<a.indexOf("?show_all_images")||-1<a.indexOf("products.json")||$.ajax({type:"DELETE",url:a,success:function(a){f(m.metafield_deleted);p()},error:function(a){f("Failed to delete",!0)}})});$(".metafield-content a.savemymeta").off("click").on("click",
function(){var a="string";$(this).hasClass("int")&&(a="integer");var b=$("#metafield_namespace").val(),c=$("#metafield_key").val(),d=$("#metafield_value").val(),e=$("#metafield_id").val(),b={metafield:{namespace:b,key:c,value:d,value_type:a}},a={metafield:{id:e,value:d,value_type:a}};5<e.length?(e=z(e),$.ajax({type:"PUT",url:e,dataType:"json",data:a,success:function(a){p();v(a);f("Metafield updated")}})):(e=z(),$.ajax({type:"POST",url:e,dataType:"json",data:b,success:function(a){p();f("Metafield saved")},
...
a.append(b)))},ra=function(){var a=$(".header .header__primary-actions:first");if(!a.length)return!1;$.ajax({type:"GET",url:"/admin/link_lists.json",dataType:"json",success:function(b){if(b){link_lists=b.link_lists;b="";for(var c=0,d=link_lists.length;c<d;c++)g("omega")!==link_lists[c].id.toString()&&(b+='<option value="'+link_lists[c].id+'">'+link_lists[c].title+"</option>");b=$("<select />",{"class":"header-select fadein"}).append("<option>Edit other Menu</option>",b).change(function(){var a=$(this).val();
a&&r("/admin/link_lists/"+a)});a.prepend(b)}},error:function(a){f("Error loading linklist data",!0)}})},U=function(){var a=$(".next-card__section .next-grid__cell--no-flex"),b=function(){$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){for(var b=$.param({utf8:"\u2713","link_list[handle]":"all-collections","link_list[title]":"All Collections"}),c=[],d=0,h=a.collections.length;d<h;++d){c.push({position:d+1,title:a.collections[d].title,link_type:"collection",
...
1;0<=b;b--){var c=$("#collections-results").find('a[href="/admin/collections/'+a.collections[b].id+'"]:last');0===c.find("span").length&&c.append('<span class="sku label badge badge--small badge--left-margin">'+a.collections[b].products_count+"</span>")}}}})}),e=$("<a/>",{"class":"btn fd-btn",href:"#",title:"Add tags to entire collection"}).html("Bulk edit tags").on("click",function(){u(!0,'<div><div class="clearfix em"><div class="half">Choose a collection</div><div class="half"><select data-action="collection"><option value="">Loading, please wait...</option></select></div></div><div class="clearfix em"><div class="half">Choose an action</div><div class="half"><select data-action="action"><option value="add">Add</option><option value="set">Set</option><option value="remove">Remove</option><option disabled value="toggle">Toggle</option><option value="purge" style="background:red;color:#fff">DELETE ALL</option></select></div></div><div class="clearfix em"><div class="half">Set the tag</div><div class="half"><input /></div></div><div class="half"><a class="btn" data-action="update_tags">Update tags</a></div><div class="half"><small>Add: Adds tags to the existing ones<br>Set: Replaces tags with new ones<br>Remove: Removes matching tags<br>Toggle: Future Use, disabled...</small></div></div>',
...
(c+='<option value="'+a[e].id+'">'+a[e].title+"</option>");a=$("<select />",{"class":"product-switcher header-select fadein"}).append("<option>Edit other Product</option>",c).change(function(){var a=$(this).val();a&&r("/admin/products/"+a)});a.find("option").sort(L).appendTo(a);b.prepend(a)}},error:function(a){f("Error loading linklist data",!0)}});a=$("#js-collections");if(a.length){var c=a.find(".list-of-added-collections a");if(c.length){var d=$("<a />",{"class":"btn fd-btn delete"}).text("Remove from all").on("click",
...
$("<a />",{"class":"btn fd-btn seo-hide-button"}).text(m.hide_from_sitemap).on("click",function(a){a.preventDefault();(a=$(this).attr("data-id"))?$.ajax({type:"DELETE",url:["//",location.host,location.pathname].join("")+"/metafields/"+a+".json",success:function(a){p()},error:function(a){f("Failed to delete",!0)}}):$.ajax({type:"POST",url:["//",location.host,location.pathname].join("")+"/metafields.json",dataType:"json",data:{metafield:{namespace:"seo",key:"hidden",value:1,value_type:"integer"}},success:function(a){p()},
...
a[Object.keys(a)[0]].id;"undefined"!==b&&r("/admin/collections/"+b)}catch(c){}},error:function(){f("Error duplicating.",!0)}});else{var e=0,g=window.location.pathname.split("/").pop(),m=function(a,d,e,g){"undefined"!==typeof a&&"undefined"!==typeof d&&("undefined"===typeof e&&(e=1),"undefined"===typeof g&&(g=[]),f("Duplicating collection, please wait..."),$.ajax({type:"GET",url:"/admin/collects.json?limit=250&fields=product_id&page="+e+"&collection_id="+a,success:function(h){g=g.concat(h.collects);
250*e>d?(b.custom_collection.collects=g,$.ajax({type:"POST",url:c,contentType:"application/json;charset=utf-8",data:JSON.stringify(b),success:function(a){try{var b=a[Object.keys(a)[0]].id;"undefined"!==b&&r("/admin/collections/"+b)}catch(c){}},error:function(){f("Error duplicating.",!0)}})):m(a,d,++e,g)}}))};(function(){$.ajax({type:"GET",url:"/admin/collects/count.json?collection_id="+g,success:function(a){e=a.count;0<e&&m(g,e,1)}})})()}})});c.append(d);a.append(c);b.prepend(a)}else f("ShopifyFD error : setup_articles : Header button missing",
...
a).text("Hidden in Sitemap"):$(".seo-hide-button").removeClass("active").removeAttr("data-id").text(m.hide_from_sitemap)},p=function(){var a=z();$.getJSON(a,function(a){var c="";a=a.metafields;E();if(a.length){g("m",a);$("#metacount").text(a.length).removeClass("hide");for(var d=0,e=a.length;d<e;d++)c+='<option data-type="'+a[d].value_type+'" data-id="'+a[d].id+'">'+a[d].namespace+"."+a[d].key+"</option>",n.metafields[a[d].id]={namespace:a[d].namespace,value:a[d].value,key:a[d].key},"backups"===a[d].namespace&&
...
'/admin/apps"]');a.length&&(a[0].href="/admin/apps/installed");setInterval(function(){n.cache.content=document.getElementById("content");if(!n.cache.content.classList.contains("loading")){ea();var a=K(),c=!1,d=a[0],e=a[1],a=a[2];"next"===e||"prev"===e||d===g("alpha")&&e===g("omega")?"next"!==e&&"prev"!==e&&a!=n.queryString&&("products"!==d||isNaN(e)?"admin"===d&&"products"===e&&(c=!0):c=!0):c=!0;if(c)if(n.alpha=d,n.omega=e,n.queryString=a,"customers"===d&&!isNaN(e))c=$(".ui-layout__section--secondary .ui-layout__item:last"),
c.length?(d=$('<div class="ui-layout__item"><div class="next-card"></div></div>'),d.find(".next-card").append('<div class="next-card-metafield next-card next-card--aside fadein"><section class="next-card__section"><h3 class="next-heading">Metafields <span id="metacount" class="hide">0</span></h3><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></section></div>'),c.after(d),c=$("div.metafield-content"),t(c)):f("ShopifyFD error : setup_customers : target HTML not found");
else if("articles"===d&&!isNaN(e))na();else if("blogs"===d&&!isNaN(e))targetHTML=$(".ui-layout__section--secondary .ui-layout__item:last"),targetHTML.length?(targetHTML.prepend('<div class="next-card-metafield next-card next-card--aside fadein"><section class="next-card__section"><h3 class="next-heading">Metafields <span id="metacount" class="hide">0</span></h3><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></section></div>'),c=$("div.metafield-content"),t(c)):
