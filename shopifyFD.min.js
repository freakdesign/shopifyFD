...
e=0,t=c.products.length;e<t;e++)d+='<option value="'+c.products[e].handle+'">'+c.products[e].title+"</option>";f.append(d);a.after(f).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),f=0,v=c.products.length;f<v;f++)d+='<option value="'+c.products[f].handle+
'">'+c.products[f].title+"</option>";e.append(d);a.after(e).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),d="",f=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),e=0,t=c.collections.length;e<t;e++)d+='<option value="'+c.collections[e].handle+
'">'+c.collections[e].title+"</option>";f.append(d);a.after(f).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),f=0,v=c.collections.length;f<v;f++)d+='<option value="'+c.collections[f].handle+'">'+c.collections[f].title+"</option>";
...
return!1}),l=$("<a/>",{"class":"btn",href:"#",title:"Create a linklist with all vendors"}).text("Create Vendor linklist").on("click",function(){e();return!1}),n=$("<p/>",{"class":"box warning",style:"margin-top:1em"}).text("If you use the trash can button to remove a linklist some ShopifyFD features will not reload. Navigate away from the page, and back again.");a&&a.append("<br><br>",h,"<br><br>",g,"<br><br>",l).find("p:eq(0)").append(n)},setup_collections:function(){b.data("collections",!1);var a=
...
dataType:"json",success:function(a){d.addClass("disabled");s(a,0,l.val(),function(){b.notice("Done. Tags have been added");d.text("Update tags").removeClass("disabled")})}}),"purge"!=c.val()&&"set"!=c.val()||$.ajax({type:"GET",url:"/admin/products.json?collection_id="+g.val()+"&fields=id",dataType:"json",success:function(a){d.addClass("disabled");n(a,0,l.val(),function(){b.notice("Done.");d.text("Update tags").removeClass("disabled")})}}),"remove"==c.val()&&$.ajax({type:"GET",url:"/admin/products.json?collection_id="+
g.val()+"&fields=id,tags",dataType:"json",success:function(a){d.addClass("disabled");r(a,0,l.val(),function(){b.notice("Matched tags have been removed.");d.text("Update tags").removeClass("disabled")})}})):b.notice("Choose a collection",!0);return!1});c.hide();$.ajax({type:"GET",url:"/admin/collections.json",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var d="",e=0,s=a.collections.length;e<s;e++)d+='<option value="'+a.collections[e].id+'">'+a.collections[e].title+
...
g;for(g in e){var h=e[g],l;for(l in h)h.hasOwnProperty(l)&&f.push({variant:{id:h[l],compare_at_price:d}})}f.length&&b.updateVariant(f,0,a)}else b.notice("Unexpected variant found.",!0);else b.notice("You only have 1 variant. No need to bulk edit",!0)});h.append(g);f.append('<i class="ico ico-20-svg ico-sidebar-bottom-website"></i>');a.append(f);b.loadmeta(d,k);$("#product-inner-variants th:last-child").before("<th>Variant ID</th>");$("th.inventory.tc").text("#").prop("title","Quantity");$("tr.variant").each(function(b){var a=
