...
f=0,u=c.products.length;f<u;f++)d+='<option value="'+c.products[f].handle+'">'+c.products[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",c);for(var d="",f=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),e=0,n=c.products.length;e<n;e++)d+='<option value="'+c.products[e].handle+
'">'+c.products[e].title+"</option>";f.append(d);a.after(f).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),f=0,u=c.collections.length;f<u;f++)d+='<option value="'+c.collections[f].handle+
'">'+c.collections[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",f=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),e=0,n=c.collections.length;e<n;e++)d+='<option value="'+c.collections[e].handle+'">'+c.collections[e].title+"</option>";
f.append(d);a.after(f).hide()}},error:function(){b.notice("Failed to load collections",!0)}});return!1});$("#metafieldselect").change(function(){var a=$(this).find(":selected");if(a.attr("data-id")){var d=c.metafields[a.attr("data-id")];a.attr("data-type");$("#metafield_namespace").val(d.namespace).prop("disabled",!0);$("#metafield_key").val(d.key).prop("disabled",!0);$("#metafield_value").val(d.value);$("#metafield_id").val(a.attr("data-id"));$("#shopifyjs_deletemetafield").removeClass("hidden")}else b.clearmetaform()});
