...
function(){var a=$(this);if(b.data("products")){for(var c=b.data("products"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),f=0,s=c.products.length;f<s;f++)d+='<option value="'+c.products[f].handle+'">'+c.products[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",
c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),s=0,f=c.products.length;s<f;s++)d+='<option value="'+c.products[s].handle+'">'+c.products[s].title+"</option>";e.append(d);a.after(e).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),
d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),f=0,s=c.collections.length;f<s;f++)d+='<option value="'+c.collections[f].handle+'">'+c.collections[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=
$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),s=0,f=c.collections.length;s<f;s++)d+='<option value="'+c.collections[s].handle+'">'+c.collections[s].title+"</option>";e.append(d);a.after(e).hide()}},error:function(){b.notice("Failed to load collections",!0)}});return!1});$("#metafieldselect").change(function(){var a=$(this).find(":selected");if(a.attr("data-id")){var d=c.metafields[a.attr("data-id")];a.attr("data-type");$("#metafield_namespace").val(d.namespace).prop("disabled",
...
"Edit tags for all products in a collection",!0);var a=$("#fdmodal"),c=a.find('select[data-action="action"]'),d=a.find("a").eq(1),e=a.find('select[data-action="collection"]'),f=a.find("input").eq(0);c.change(function(b){"purge"==c.val()||"set"==c.val()?("purge"==c.val()&&f.val("").attr("disabled","disabled"),d.addClass("delete")):(f.removeAttr("disabled"),d.removeClass("delete"))});var v=function(b,a,c,e){var f=b.products[a].id,h={product:{id:f,tags:c}};d.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",
url:"/admin/products/"+f+".json",data:h,dataType:"json",success:function(d){++a;a<b.products.length?v(b,a,c,e):"function"===typeof e&&e()}})},w=function(b,a,c,e){if(b.products[a].tags){var f=c.replace(/, /g,",").split(","),h=b.products[a].tags.replace(/, /g,",").split(","),h=$(h).not(f).get()+"",f=b.products[a].id,h={product:{id:f,tags:h}};d.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",url:"/admin/products/"+f+".json",data:h,dataType:"json",success:function(d){++a;a<b.products.length?w(b,a,
c,e):"function"===typeof e&&e()}})}else++a,a<b.products.length?w(b,a,c,e):"function"===typeof e&&e()},t=function(a,c,e,f){var h=e.replace(/, /g,",").split(","),g=a.products[c].tags.split(","),g=b.array_unique(h.concat(g))+"",h=a.products[c].id,g={product:{id:h,tags:g}};d.text(c+1+"/"+a.products.length);$.ajax({type:"PUT",url:"/admin/products/"+h+".json",data:g,dataType:"json",success:function(b){++c;c<a.products.length?t(a,c,e,f):f()}})};d.on("click",function(){e.val().length?("add"===c.val()&&f.val().length&&
$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id,tags",dataType:"json",success:function(a){d.addClass("disabled");t(a,0,f.val(),function(){b.notice("Done. Tags have been added");d.text("Update tags").removeClass("disabled")})}}),"purge"!==c.val()&&"set"!==c.val()||$.ajax({type:"GET",url:"/admin/products.json?collection_id="+e.val()+"&fields=id",dataType:"json",success:function(a){d.addClass("disabled");v(a,0,f.val(),function(){b.notice("Done.");d.text("Update tags").removeClass("disabled")})}}),
...
b.data("countries").countries.length){var d=[],e=[],f=[],h=function(a,c,d){if(c.length){var e=[];$.ajax({type:"GET",url:"/admin/countries/"+a+".json",dataType:"json",success:function(a){var f=a.country.weight_based_shipping_rates,h=a.country.price_based_shipping_rates;a=a.country.carrier_shipping_rate_providers;for(var g=0,u=f.length;g<u;g++)e.push([f[g].id,"weight_based"]);g=0;for(u=h.length;g<u;g++)e.push([h[g].id,"price_based"]);g=0;for(u=a.length;g<u;g++)e.push([a[g].id,"carrier_based"]);g=0;
for(u=e.length;g<u;g++)$.ajax({type:"POST",url:"/admin/shipping_rates/"+e[g][0],data:{utf8:"\u2713",_method:"delete",shipping_rate:{type:e[g][1]}},success:function(b){},error:function(a){b.notice("Failed to replace rate",!0)}});b.save_new_rates_FIX(c,0,d)},fail:function(){b.notice("I failed to get the country.json",!0)}})}else b.notice("Missing items to add",!0)};$("div.shipping-rate-table").each(function(a){var c=$(this),p=$("<a/>",{href:"#","data-country":b.data("countries").countries[a].name,"data-cid":b.data("countries").countries[a].id,
...
!0)},get_files:function(a,c){var d=$("#settings-general table");d.length&&(b.notice("Loading files..."),$.ajax({type:"GET",url:"/admin/files.json?limit=250&direction=next&page="+a,success:function(a,f,h){if(a){$("");f=0;for(h=a.files.length;f<h;f++){var g=$("<tr />",{"class":"file_row"}),k=$("<td />"),p=$("<td />"),n=$("<td />"),l=$("<td />",{"class":"tr"}),r=$("<td />"),t=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:","")});if(-1<a.files[f].content_type.indexOf("image")){t.on("click",
function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var s=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");k.html('<img src="'+s+'" />');p.append(t);n.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else t.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",""));l.html(Math.floor(a.files[f].size/1024)+
...
typeof b)return r[a];r[a]=b},toggleStyle:function(){$("html").toggleClass("shopifyJSoverride")},isloading:function(){return b.data("content").hasClass("loading")}}}();b.load_css();b.init()}else alert("Error. This should be run within the Shopify Admin page.")}else alert("Error. jQuery not found.")})();