(function(){if("undefined"!==typeof Shopify)if("undefined"!==typeof jQuery)if($("#shopifyjs").length)"function"===typeof Shopify.Flash.error&&Shopify.Flash.error("Error. ShopifyFD already loaded!");else{var l=document;if(1<l.URL.indexOf("myshopify.com/admin")){var x=$("<div />",{"class":"box error animated fadein"}).html('Warning - Some ShopifyFD features do not currently work on this view. Fixes are being looked at, but for now expect errors. See this <a target="_blank" href="https://ecommerce.shopify.com/c/shopify-apis-and-technology/t/dashboard-endpoints-failing-link-lists-shipping-files-etc-212161">related post</a>.'),
b=function(){var q={debug:!1,drag_on:!1,alpha:!1,omega:!1,countries:!1,wait:1E3,api_count:0,api_limit:250,metafields:{},autosave:!1,content:$("#content"),body:$("body")};return{escape:function(b){return b.replace(/[\\]/g,"\\\\").replace(/[\"]/g,'\\"').replace(/[\/]/g,"\\/").replace(/[\b]/g,"\\b").replace(/[\f]/g,"\\f").replace(/[\n]/g,"\\n").replace(/[\r]/g,"\\r").replace(/[\t]/g,"\\t")},redirect:function(b){if("undefined"!==typeof b&&"function"===typeof Turbolinks)try{Turbolinks.visit(b)}catch(c){return!1}},
...
function(){var a=$(this);if(b.data("products")){for(var c=b.data("products"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),n=0,f=c.products.length;n<f;n++)d+='<option value="'+c.products[n].handle+'">'+c.products[n].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",
c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),u=0,n=c.products.length;u<n;u++)d+='<option value="'+c.products[u].handle+'">'+c.products[u].title+"</option>";e.append(d);a.after(e).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),
d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),n=0,f=c.collections.length;n<f;n++)d+='<option value="'+c.collections[n].handle+'">'+c.collections[n].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=
$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),n=0,f=c.collections.length;n<f;n++)d+='<option value="'+c.collections[n].handle+'">'+c.collections[n].title+"</option>";e.append(d);a.after(e).hide()}},error:function(){b.notice("Failed to load collections",!0)}});return!1});$("#metafieldselect").change(function(){var a=$(this).find(":selected");if(a.attr("data-id")){var d=c.metafields[a.attr("data-id")];a.attr("data-type");$("#metafield_namespace").val(d.namespace).prop("disabled",
...
url:c,success:function(a){b.flog(a);b.notice("Metafield deleted");b.updatedropdown()},error:function(a){b.notice("Failed to delete",!0)}})});$("div.sub_section-summary a.savemymeta").off("click").on("click",function(){var a="string";$(this).hasClass("int")&&(a="integer");var c=$("#metafield_namespace").val(),d=$("#metafield_key").val(),e=$("#metafield_value").val(),n=$("#metafield_id").val(),c={metafield:{namespace:c,key:d,value:e,value_type:a}},a={metafield:{id:n,value:e,value_type:a}};5<n.length?
(e=l.URL+"/metafields/"+n+".json","articles"===b.data("alpha")&&(e="/admin/articles/"+b.data("omega")+"/metafields/"+n+".json"),"general"===b.data("omega")&&(e="/admin/metafields/"+n+".json"),$.ajax({type:"PUT",url:e,dataType:"json",data:a,success:function(a){b.updatedropdown();b.flog(a);b.notice("Metafield updated")}})):(e=l.URL+"/metafields.json","general"===b.data("omega")&&(e="/admin/metafields.json"),"articles"===b.data("alpha")&&(e="/admin/articles/"+b.data("omega")+"/metafields.json"),$.ajax({type:"POST",
...
title:"All Collections",links:[]}},c=0,d=b.collections.length;c<d;++c)a.link_list.links.push({position:c+1,title:b.collections[c].title,link_type:"collection",subject_id:b.collections[c].id});g(a)},error:function(){b.notice("Error! Are you sure you have access to collections?",!0)}});return!1},d=function(){$.ajax({type:"GET",url:"/admin/pages.json?limit=250",dataType:"json",success:function(b){for(var a={link_list:{handle:"all-pages",title:"All Pages",links:[]}},c=0,d=b.pages.length;c<d;++c)a.link_list.links.push({position:c+
1,title:b.pages[c].title,link_type:"page",subject_id:b.pages[c].id});g(a)},error:function(){b.notice("Error! Are you sure you have access to pages?",!0)}});return!1},e=function(){$.ajax({type:"GET",url:"/admin/products/vendors.json",dataType:"json",success:function(b){for(var a=$.param({utf8:"\u2713","link_list[handle]":"all-vendors","link_list[title]":"All Vendors"}),c=[],d=0,e=b.vendors.length;d<e;++d){var g={position:d+1,title:b.vendors[d],link_type:"http",subject:"/collections/vendors?q="+encodeURIComponent(b.vendors[d].toLowerCase()).replace(/%20/g,
"+")},k={};k["link_list[links][][position]"]=d+1;k["link_list[links][][title]"]=b.vendors[d];k["link_list[links][][link_type]"]="http";k["link_list[links][][subject]"]="/collections/vendors?q="+encodeURIComponent(b.vendors[d].toLowerCase()).replace(/%20/g,"+");c.push(g);a+="&"+$.param(k)}f(a)}})},f=function(a){"undefined"!==typeof a&&$.ajax({type:"POST",url:"/admin/link_lists",data:a,success:function(a,c,d){(c=d.getResponseHeader("X-XHR-Redirected-To"))?(linklistID=c.split("/").pop(),isNaN(linklistID)?
b.notice("Error creating linklist - ID not returned",!0):(b.notice("Link list added"),b.redirect("/admin/link_lists/"+linklistID))):(linklistID=$(a).find("form").eq(0).attr("action").split("/").pop(),isNaN(linklistID)||(b.notice("Link list added"),b.redirect("/admin/link_lists/"+linklistID)))},error:function(){b.notice("Error creating linklist",!0)}})},g=function(a){$.ajax({type:"POST",url:"/admin/link_lists.json",data:JSON.stringify(a),contentType:"application/json;charset=utf-8",success:function(a){b.notice("Link list added");
b.redirect("/admin/link_lists/"+a.link_list.id)},error:function(a){406===a.status?(b.notice("Linklist creation attempted. Reload page."),b.redirect("/admin/links")):b.notice("Error creating linklist",!0)}})},h=$("<a/>",{href:"#","class":"tooltip-bottom tooltip",style:"margin-right:1.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Make a copy of this linklist</span></span>Copy').on("click",function(a){a.preventDefault();a=$(this).parent().find('a[href^="/admin/link_lists"]').eq(0);
a.length?(href=a.attr("href").split("/").pop(),isNaN(href)||$.ajax({type:"GET",url:"/admin/link_lists/"+href,success:function(b){var a=$(b).find("#edit_link_list_"+href);a.length&&(a.find('input[name="_method"]').remove(),a.find('input[name="authenticity_token"]').remove(),b=a.find('input[name="link_list[title]"]'),b.val(b.val()+" [COPY]"),a.find('select[name="link_list[links][][link_type]"]').each(function(b){var c=$(this).val();"collection"===c?(b=a.find('div[context="collectionPicker'+b+'"]').eq(0),
c=b.attr("define").split(","),"undefined"!==typeof c[2]&&(c=parseInt(c[2].replace(/\D/g,"")),b.after('<input name="link_list[links][][subject_id]" value="'+c+'" />'))):"page"===c?(b=a.find('div[context="pagePicker'+b+'"]').eq(0),c=b.attr("define").split(","),"undefined"!==typeof c[2]&&(c=parseInt(c[2].replace(/\D/g,"")),b.after('<input name="link_list[links][][subject_id]" value="'+c+'" />'))):"product"===c&&(b=a.find('div[context="productPicker'+b+'"]').eq(0),c=b.attr("define").split(","),"undefined"!==
typeof c[2]&&(c=parseInt(c[2].replace(/\D/g,"")),b.after('<input name="link_list[links][][subject_id]" value="'+c+'" />')))}),f(a.serialize()))},error:function(a,c,d){b.notice(d,!0)}})):b.notice("Can not copy this linklist. ID was not found",!0)});a.prepend(h);var a=$(".section-summary").eq(0),h=$("<a/>",{"class":"btn tooltip-bottom tooltip",style:"margin-top:2em",href:"#"}).html('<span class="tooltip-container"><span class="tooltip-label">Create a linklist with every collection</span></span>Create Collections linklist').on("click",
function(){c();return!1}),k=$("<a/>",{"class":"btn tooltip-bottom tooltip",href:"#"}).html('<span class="tooltip-container"><span class="tooltip-label">Create a linklist with every page</span></span>Create Pages linklist').on("click",function(){d();return!1}),p=$("<a/>",{"class":"btn tooltip-bottom tooltip",href:"#"}).html('<span class="tooltip-container"><span class="tooltip-label">Create a linklist with all vendors</span></span>Create Vendor linklist').on("click",function(){e();return!1});a&&a.append("<br>",
h,"<br>",k,"<br>",p);$(".section.link-lists").eq(0).prepend(x);a=$('.section-summary a[href="/admin/link_lists/new"]');if(a.length&&(h=$("#link_lists .next-grid__cell header a:last-child"),h.length)){var s="";2<h.length&&(h.each(function(){var b=$(this),a=b.parent().parent().find("h3").text(),b=b.attr("href");a.length&&b.length&&(s+='<option value="'+b+'">'+a+"</option>")}),s.length&&(h=$("<select />",{style:"margin-top:1em;border-radius:3px;border-color: #D3D3D3;color: #479ccf;"}).on("change",function(){var a=
$(this).val();a.length&&b.redirect(a)}).html('<option value="">Jump to linklist</option>'+s),k=$("<div />"),k.append(h,'<small style="display: block">Select a linklist to quickly jump to edit view. Useful for when you have many linklists...</small>'),a.after(k)))}},setup_collections:function(){b.data("collections",!1);var a=$(".header-right").eq(0);if(a.length){var c=$("<ul/>",{"class":"segmented"}),d=$("<li/>"),e=$("<a/>",{"class":"btn",style:"margin-left:.5em",href:"#"}).text("Show Product Count").on("click",
function(a){a.preventDefault();$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var c=a.collections.length-1;0<=c;c--){var d=$("#collections-results").find('a[href="/admin/collections/'+a.collections[c].id+'"]');0===d.find("span").length&&d.append('<span class="sku label">'+a.collections[c].products_count+"</span>")}}}})}),f=$("<a/>",{"class":"btn",href:"#",title:"Add tags to entire collection"}).html("Bulk edit tags").on("click",
...
f;for(f in g){var h=g[f],k;for(k in h)h.hasOwnProperty(k)&&e.push({variant:{id:h[k],compare_at_price:d}})}e.length&&b.updateVariant(e,0,a)}else b.notice("Unexpected variant found.",!0);else b.notice("You only have 1 variant. No need to bulk edit",!0)});d.append(k)}else b.notice("ShopifyFD error : setup_products : Inventory target HTML not found",!0);$("#product-inner-variants th:last-child").length?($("#product-inner-variants th:last-child").before("<th>Variant ID</th>"),$("th.inventory.tc").text("#").prop("title",
...
d<e.length?f(d,e):(b.notice("Weight updated"),a.removeClass("is-loading").text("Save"))}})};a.addClass("is-loading").text("");f(0,e)}else b.notice("Did you set a weight?",!0);return!1});return!1});$("<span/>",{"class":"hidden"});a.append(s);$('tr.variant input[type="checkbox"]').on("change",function(){1<$('tr.variant input[type="checkbox"]:checked').length?s.removeClass("hidden"):s.addClass("hidden")})}else b.notice("ShopifyFD error : setup_products : unable to add bulk weight editor");d.length&&
"object"===typeof g&&1<Object.keys(g).length&&h.after(p)},setup_pages:function(){var a=$(".section.description .section-summary").eq(0);if(a.length){var c=a.find("a").eq(0).attr("href");a.after('<div class="sub_section-summary fadein"><h1>Metafields <span id="metacount" class="animated bounce">0</span></h1><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></div>').remove();a=$("div.metafield-content");b.loadmeta(a,q)}else b.notice("ShopifyFD error : setup_pages : Metafield target HTML not found",
...
$(".section.shipping-rates").eq(0).prepend(a);a=$("#settings-shipping header").eq(0);if(a.length){var c=$('<div class="header-right animated fadein"><div class="header-action"><a class="btn btn-separate tooltip-bottom tooltip bulk-options" href="#"><span class="tooltip-container"><span class="tooltip-label">Show bulk delete options</span></span>Bulk Delete Options</a> <a class="btn btn-separate tooltip-bottom tooltip export-as-json" href="#"><span class="tooltip-container"><span class="tooltip-label">Export countries</span></span>Export JSON</a></div></div>');
...
style:"float:right;margin-right:.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Will replace ALL existing rates.</span></span>Paste rates (replace)').on("click",function(){var a=$(this),b=a.attr("data-cid"),c=d,h=e,k=f,p=[],l=0;a.parent().parent().find("tbody").remove();a.parent().find("a").addClass("hidden");for(var r=0,q=c.length;r<q;r++)c[r].country_id=b,delete c[r].id,c[r].type="weight",p[l]=c[r],l++;r=0;for(q=h.length;r<q;r++)h[r].country_id=b,delete h[r].id,h[r].type=
"price",p[l]=h[r],l++;r=0;for(q=k.length;r<q;r++)k[r].country_id=b,delete k[r].id,k[r].type="carrier",p[l]=k[r],l++;g(b,p,a);return!1});c=c.find('a[bind-event-click="addShippingRateModal.show()"]');if(c.length)c.after(p,a);else return b.notice("Error. Unable to add copy button.",!0),!1})}else b.notice("Mismatch in country count. Reload this page.",!0)}else b.notice("Loading current country list"),$.ajax({type:"GET",url:"/admin/countries.json",dataType:"json",success:function(a){a.countries.length&&
...
a=$("div.metafield-content"),b.loadmeta(a,q),0===$("#rte_extra").length&&($("#collection-description_iframecontainer").eq(0).after('<div id="rte_extra"><a class="btn btn-slim tooltip tooltip-bottom" id="clearformatting" href="#"><span class="tooltip-container"><span class="tooltip-label">Careful, this method is brutal...</span></span>Purge html</a> <a class="btn btn-slim tooltip tooltip-bottom" id="createbackup" href="#"><span class="tooltip-container"><span class="tooltip-label">Save contents as metafield</span></span>Create Backup</a> <a class="btn btn-slim" style="display:none;" id="restorebackup" href="#">Restore Backup</a> <a class="btn btn-slim tooltip tooltip-bottom" id="save_images_to_meta" href="#"><span class="tooltip-container"><span class="tooltip-label">Add image paths to a metafield</span></span>Images to Metafields</a></div>'),
b.setup_rte())):b.notice("ShopifyFD error : setup_custom_collections : target html not found",!0)},get_files:function(a,c){var d=$("#settings-general table");b.notice("Loading files...");$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),k=$("<td />"),p=$("<td />"),s=$("<td />"),l=$("<td />",{"class":"tr"}),q=$("<td />"),t=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:",
"")});if(-1<a.files[f].content_type.indexOf("image")){t.on("click",function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var n=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");k.html('<img src="'+n+'" />');p.append(t);s.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else t.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",
""));l.html(Math.floor(a.files[f].size/1024)+" kB");h.append(k,p,s,l,q);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}})},setup_files:function(){var a=$(".header-row .header-right").eq(0);if(a.length){var c=$("<ul/>",{"class":"segmented",id:"get_all_images"}),d=$("<li/>"),e=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");
...
typeof b)return q[a];q[a]=b},toggleStyle:function(){$("html").toggleClass("shopifyJSoverride")},isloading:function(){return b.data("content").hasClass("loading")}}}();b.load_css();b.init()}else alert("Error. This should be run within the Shopify Admin page.")}else alert("Error. jQuery not found.")})();