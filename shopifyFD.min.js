(function(){if("undefined"!==typeof Shopify)if("undefined"!==typeof jQuery)if($("#shopifyjs").length)"function"===typeof Shopify.Flash.error&&Shopify.Flash.error("Error. ShopifyFD already loaded!");else{var p=document;if(1<p.URL.indexOf("myshopify.com/admin")){var b=function(){var s={debug:!1,drag_on:!1,alpha:!1,omega:!1,countries:!1,wait:1E3,api_count:0,api_limit:250,metafields:{},autosave:!1,content:$("#content"),body:$("body")};return{escape:function(b){return b.replace(/[\\]/g,"\\\\").replace(/[\"]/g,
...
function(){var a=$(this);if(b.data("products")){for(var c=b.data("products"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),l=0,f=c.products.length;l<f;l++)d+='<option value="'+c.products[l].handle+'">'+c.products[l].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",
c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),l=0,f=c.products.length;l<f;l++)d+='<option value="'+c.products[l].handle+'">'+c.products[l].title+"</option>";e.append(d);a.after(e).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),
d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),l=0,f=c.collections.length;l<f;l++)d+='<option value="'+c.collections[l].handle+'">'+c.collections[l].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=
$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),l=0,f=c.collections.length;l<f;l++)d+='<option value="'+c.collections[l].handle+'">'+c.collections[l].title+"</option>";e.append(d);a.after(e).hide()}},error:function(){b.notice("Failed to load collections",!0)}});return!1});$("#metafieldselect").change(function(){var a=$(this).find(":selected");if(a.attr("data-id")){var d=c.metafields[a.attr("data-id")];a.attr("data-type");$("#metafield_namespace").val(d.namespace).prop("disabled",
...
url:c,success:function(a){b.flog(a);b.notice("Metafield deleted");b.updatedropdown()},error:function(a){b.notice("Failed to delete",!0)}})});$("div.sub_section-summary a.savemymeta").off("click").on("click",function(){var a="string";$(this).hasClass("int")&&(a="integer");var c=$("#metafield_namespace").val(),d=$("#metafield_key").val(),e=$("#metafield_value").val(),l=$("#metafield_id").val(),c={metafield:{namespace:c,key:d,value:e,value_type:a}},a={metafield:{id:l,value:e,value_type:a}};5<l.length?
(e=p.URL+"/metafields/"+l+".json","articles"===b.data("alpha")&&(e="/admin/articles/"+b.data("omega")+"/metafields/"+l+".json"),"general"===b.data("omega")&&(e="/admin/metafields/"+l+".json"),$.ajax({type:"PUT",url:e,dataType:"json",data:a,success:function(a){b.updatedropdown();b.flog(a);b.notice("Metafield updated")}})):(e=p.URL+"/metafields.json","general"===b.data("omega")&&(e="/admin/metafields.json"),"articles"===b.data("alpha")&&(e="/admin/articles/"+b.data("omega")+"/metafields.json"),$.ajax({type:"POST",
...
if(c.length){var e=function(a,c){$.ajax({type:"GET",url:"/admin/products/"+a[c]+".json?fields=variants",dataType:"json",success:function(f){var t=f.product.variants[0].sku;f=f.product.variants[0].id;var g="";if(t||f)t&&(g='<span title="SKU" class="sku label animated fadein">'+t+"</span>"),d.eq(c).before(g+'<span title="VariantID" class="variant label animated fadein">'+f+"</span>");c+1<a.length?e(a,c+1):b.notice("SKUs and Variant IDs Loaded")}})};b.notice("Loading SKUs and Variant IDs, please wait...");
...
$.param({utf8:"\u2713","link_list[handle]":"all-collections","link_list[title]":"All Collections"}),c=[],d=0,e=b.collections.length;d<e;++d){c.push({position:d+1,title:b.collections[d].title,link_type:"collection",subject_id:b.collections[d].id});var h={};h["link_list[links][][position]"]=d+1;h["link_list[links][][title]"]=b.collections[d].title;h["link_list[links][][link_type]"]="collection";h["link_list[links][][subject_id]"]=b.collections[d].id;a+="&"+$.param(h)}f(a)},error:function(){b.notice("Error! Are you sure you have access to collections?",
!0)}});return!1},d=function(){$.ajax({type:"GET",url:"/admin/pages.json?limit=250",dataType:"json",success:function(b){for(var a=$.param({utf8:"\u2713","link_list[handle]":"all-pages","link_list[title]":"All Pages"}),c=[],d=0,e=b.pages.length;d<e;++d){c.push({position:d+1,title:b.pages[d].title,link_type:"page",subject_id:b.pages[d].id});var h={};h["link_list[links][][position]"]=d+1;h["link_list[links][][title]"]=b.pages[d].title;h["link_list[links][][link_type]"]="page";h["link_list[links][][subject_id]"]=
b.pages[d].id;a+="&"+$.param(h)}f(a)},error:function(){b.notice("Error! Are you sure you have access to pages?",!0)}});return!1},e=function(){$.ajax({type:"GET",url:"/admin/products/vendors.json",dataType:"json",success:function(b){for(var a=$.param({utf8:"\u2713","link_list[handle]":"all-vendors","link_list[title]":"All Vendors"}),c=[],d=0,e=b.vendors.length;d<e;++d){var h={position:d+1,title:b.vendors[d],link_type:"http",subject:"/collections/vendors?q="+encodeURIComponent(b.vendors[d].toLowerCase()).replace(/%20/g,
"+")},g={};g["link_list[links][][position]"]=d+1;g["link_list[links][][title]"]=b.vendors[d];g["link_list[links][][link_type]"]="http";g["link_list[links][][subject]"]="/collections/vendors?q="+encodeURIComponent(b.vendors[d].toLowerCase()).replace(/%20/g,"+");a+="&"+$.param(g);c.push(h)}f(a)}})},f=function(a){"undefined"!==typeof a&&$.ajax({type:"POST",url:"/admin/link_lists",data:a,success:function(a,c,d){(c=d.getResponseHeader("X-XHR-Redirected-To"))?(linklistID=c.split("/").pop(),isNaN(linklistID)?
...
"object"===typeof h&&1<Object.keys(h).length&&k.after(q)},setup_pages:function(){var a=$(".section.description .section-summary").eq(0);if(a.length){var c=a.find("a").eq(0).clone(!0),d=c.attr("href");a.after('<div class="sub_section-summary fadein"><h1>Metafields <span id="metacount" class="animated bounce">0</span></h1><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></div>').remove();a=$("div.metafield-content");b.loadmeta(a,s)}else b.notice("ShopifyFD error : setup_pages : Metafield target HTML not found",
...
style:"float:right;margin-right:.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Will replace ALL existing rates.</span></span>Paste rates (replace)').on("click",function(){var b=$(this),a=b.attr("data-cid"),c=d,h=e,k=f,n=[],q=0;b.parent().parent().find("tbody").remove();b.parent().find("a").addClass("hidden");for(var r=0,p=c.length;r<p;r++)c[r].country_id=a,delete c[r].id,c[r].type="weight",n[q]=c[r],q++;r=0;for(p=h.length;r<p;r++)h[r].country_id=a,delete h[r].id,h[r].type=
"price",n[q]=h[r],q++;r=0;for(p=k.length;r<p;r++)k[r].country_id=a,delete k[r].id,k[r].type="carrier",n[q]=k[r],q++;g(a,n,b);return!1});c=c.find('a[bind-event-click="addShippingRateModal.show()"]');if(c.length)c.after(n,a,q);else return b.notice("Error. Unable to add copy button.",!0),!1})}else b.notice("Mismatch in country count. Reload this page.",!0)}else b.notice("Loading current country list"),$.ajax({type:"GET",url:"/admin/countries.json",dataType:"json",success:function(a){a.countries.length&&
...
b.setup_rte())):b.notice("ShopifyFD error : setup_custom_collections : target html not found",!0)},get_files:function(a,c){var d=$("#settings-general table");d.length&&(b.notice("Loading files..."),$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),k=$("<td />"),n=$("<td />"),q=$("<td />"),p=$("<td />",{"class":"tr"}),s=$("<td />"),w=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:",
"")});if(-1<a.files[f].content_type.indexOf("image")){w.on("click",function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var l=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");k.html('<img src="'+l+'" />');n.append(w);q.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else w.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",
""));p.html(Math.floor(a.files[f].size/1024)+" kB");h.append(k,n,q,p,s);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}}))},setup_files:function(){var a=$(".header-row .header-right").eq(0);if(a.length){var c=$("<ul/>",{"class":"segmented",id:"get_all_images"}),d=$("<li/>"),e=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");
...
c.find(".span18"),h=$("<div />",{"class":"box box-details"}),k=$("<ol />",{"class":"content-index content-index-img-icons"});$.ajax({type:"GET",url:"https://freakdesign-us.s3.amazonaws.com/shopify/shopifyFD/s/announcements.json",dataType:"json",cache:!1,xhrFields:{withCredentials:!0},crossDomain:!0,success:function(b){for(var d=0;d<b.announcements.length;d++){var p=$("<li />",{"class":"content-index__item"}).html("<small>"+b.announcements[d].date+"</small><br><strong>"+b.announcements[d].title+"</strong><p>"+
