(function(){if("undefined"!==typeof Shopify)if("undefined"!==typeof jQuery)if($("#shopifyjs").length)"function"===typeof Shopify.Flash.error&&Shopify.Flash.error("Error. ShopifyFD already loaded!");else{var l=document;if(1<l.URL.indexOf("myshopify.com/admin")){var b=function(){var s={debug:!1,drag_on:!1,alpha:!1,omega:!1,countries:!1,wait:1E3,api_count:0,api_limit:250,metafields:{},autosave:!1,content:$("#content"),body:$("body")};return{escape:function(b){return b.replace(/[\\]/g,"\\\\").replace(/[\"]/g,
...
"/"+b.data("omega")+"/metafields.json?limit=250";"general"===b.data("omega")&&(d="/admin/metafields.json");"articles"===b.data("alpha")&&(d="/admin/articles/"+b.data("omega")+"/metafields.json");$.getJSON(d,function(e){var d="",g=b.data("datalistNamespace")||["global"],h=b.data("datalistKey")||[];if(m=e.metafields){b.data("m",m);"products"!==b.data("alpha")&&"collections"!==b.data("alpha")&&"pages"!==b.data("alpha")||b.setup_copypaste();e=$("#metacount");e.text(m.length).addClass("active");0===m.length?
e.removeClass("active"):e.addClass("active");e=0;for(var k=m.length;e<k;e++)d+='<option data-type="'+m[e].value_type+'" data-id="'+m[e].id+'">'+m[e].namespace+"."+m[e].key+"</option>",c.metafields[m[e].id]={namespace:m[e].namespace,value:m[e].value,key:m[e].key},"backups"==m[e].namespace&&(b.flog("hasbackup!"),$("#restorebackup").show(),b.data("hasbackup",!0)),g.push(m[e].namespace),h.push(m[e].key);g=b.array_unique(g);h=b.array_unique(h);b.data("datalistNamespace",g);b.data("datalistKey",h);d='<select id="metafieldselect"><option value="">Select or create a metafield</option>'+
...
function(){var a=$(this);if(b.data("products")){for(var c=b.data("products"),d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),f=0,r=c.products.length;f<r;f++)d+='<option value="'+c.products[f].handle+'">'+c.products[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/products.json?limit=250",dataType:"json",success:function(c){if(c.products.length){b.data("products",
c);for(var d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add product handle as value</option>'),r=0,f=c.products.length;r<f;r++)d+='<option value="'+c.products[r].handle+'">'+c.products[r].title+"</option>";e.append(d);a.after(e).hide()}},fail:function(){b.notice("Failed to load products",!0)}});return!1});$("#adv_get_collections").off("click").on("click",function(){var a=$(this);if(b.data("collections")){for(var c=b.data("collections"),
d="",e=$("<select />",{}).change(function(){var b=$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),f=0,r=c.collections.length;f<r;f++)d+='<option value="'+c.collections[f].handle+'">'+c.collections[f].title+"</option>";e.append(d);a.after(e).hide()}else $.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(c){if(c.collections.length){b.data("collections",c);for(var d="",e=$("<select />",{}).change(function(){var b=
$(this);$("#metafield_value").val(b.val())}).html('<option value="">Add collection handle as value</option>'),r=0,f=c.collections.length;r<f;r++)d+='<option value="'+c.collections[r].handle+'">'+c.collections[r].title+"</option>";e.append(d);a.after(e).hide()}},error:function(){b.notice("Failed to load collections",!0)}});return!1});$("#metafieldselect").change(function(){var a=$(this).find(":selected");if(a.attr("data-id")){var d=c.metafields[a.attr("data-id")];a.attr("data-type");$("#metafield_namespace").val(d.namespace).prop("disabled",
...
$.param({utf8:"\u2713","link_list[handle]":"all-collections","link_list[title]":"All Collections"}),c=[],e=0,d=b.collections.length;e<d;++e){c.push({position:e+1,title:b.collections[e].title,link_type:"collection",subject_id:b.collections[e].id});var h={};h["link_list[links][][position]"]=e+1;h["link_list[links][][title]"]=b.collections[e].title;h["link_list[links][][link_type]"]="collection";h["link_list[links][][subject_id]"]=b.collections[e].id;a+="&"+$.param(h)}f(a)},error:function(){b.notice("Error! Are you sure you have access to collections?",
!0)}});return!1},d=function(){$.ajax({type:"GET",url:"/admin/pages.json?limit=250",dataType:"json",success:function(b){for(var a=$.param({utf8:"\u2713","link_list[handle]":"all-pages","link_list[title]":"All Pages"}),c=[],e=0,d=b.pages.length;e<d;++e){c.push({position:e+1,title:b.pages[e].title,link_type:"page",subject_id:b.pages[e].id});var h={};h["link_list[links][][position]"]=e+1;h["link_list[links][][title]"]=b.pages[e].title;h["link_list[links][][link_type]"]="page";h["link_list[links][][subject_id]"]=
b.pages[e].id;a+="&"+$.param(h)}f(a)},error:function(){b.notice("Error! Are you sure you have access to pages?",!0)}});return!1},e=function(){$.ajax({type:"GET",url:"/admin/products/vendors.json",dataType:"json",success:function(b){for(var a=$.param({utf8:"\u2713","link_list[handle]":"all-vendors","link_list[title]":"All Vendors"}),c=[],e=0,d=b.vendors.length;e<d;++e){var h={position:e+1,title:b.vendors[e],link_type:"http",subject:"/collections/vendors?q="+encodeURIComponent(b.vendors[e].toLowerCase()).replace(/%20/g,
"+")},g={};g["link_list[links][][position]"]=e+1;g["link_list[links][][title]"]=b.vendors[e];g["link_list[links][][link_type]"]="http";g["link_list[links][][subject]"]="/collections/vendors?q="+encodeURIComponent(b.vendors[e].toLowerCase()).replace(/%20/g,"+");a+="&"+$.param(g);c.push(h)}f(a)}})},f=function(a){"undefined"!==typeof a&&$.ajax({type:"POST",url:"/admin/link_lists",data:a,success:function(a,c,e){(c=e.getResponseHeader("X-XHR-Redirected-To"))?(linklistID=c.split("/").pop(),isNaN(linklistID)?
...
href:"#"}).text("Show Product Count").on("click",function(a){a.preventDefault();$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var c=a.collections.length-1;0<=c;c--){var e=$("#collections-results").find('a[href="/admin/collections/'+a.collections[c].id+'"]');0===e.find("span").length&&e.append('<span class="sku label">'+a.collections[c].products_count+"</span>")}}}})}),f=$("<a/>",{"class":"btn",href:"#",
...
"Edit tags for all products in a collection",!0);var a=$("#fdmodal"),c=a.find('select[data-action="action"]'),e=a.find("a").eq(1),d=a.find('select[data-action="collection"]'),f=a.find("input").eq(0);c.change(function(b){"purge"==c.val()||"set"==c.val()?("purge"==c.val()&&f.val("").attr("disabled","disabled"),e.addClass("delete")):(f.removeAttr("disabled"),e.removeClass("delete"))});var v=function(b,a,c,d){var f=b.products[a].id,h={product:{id:f,tags:c}};e.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",
url:"/admin/products/"+f+".json",data:h,dataType:"json",success:function(e){++a;a<b.products.length?v(b,a,c,d):"function"===typeof d&&d()}})},u=function(b,a,c,d){if(b.products[a].tags){var f=c.replace(/, /g,",").split(","),h=b.products[a].tags.replace(/, /g,",").split(","),h=$(h).not(f).get()+"",f=b.products[a].id,h={product:{id:f,tags:h}};e.text(a+1+"/"+b.products.length);$.ajax({type:"PUT",url:"/admin/products/"+f+".json",data:h,dataType:"json",success:function(e){++a;a<b.products.length?u(b,a,
c,d):"function"===typeof d&&d()}})}else++a,a<b.products.length?u(b,a,c,d):"function"===typeof d&&d()},w=function(a,c,d,f){var h=d.replace(/, /g,",").split(","),g=a.products[c].tags.split(","),g=b.array_unique(h.concat(g))+"",h=a.products[c].id,g={product:{id:h,tags:g}};e.text(c+1+"/"+a.products.length);$.ajax({type:"PUT",url:"/admin/products/"+h+".json",data:g,dataType:"json",success:function(b){++c;c<a.products.length?w(a,c,d,f):f()}})};e.on("click",function(){d.val().length?("add"===c.val()&&f.val().length&&
$.ajax({type:"GET",url:"/admin/products.json?collection_id="+d.val()+"&fields=id,tags",dataType:"json",success:function(a){e.addClass("disabled");w(a,0,f.val(),function(){b.notice("Done. Tags have been added");e.text("Update tags").removeClass("disabled")})}}),"purge"!==c.val()&&"set"!==c.val()||$.ajax({type:"GET",url:"/admin/products.json?collection_id="+d.val()+"&fields=id",dataType:"json",success:function(a){e.addClass("disabled");v(a,0,f.val(),function(){b.notice("Done.");e.text("Update tags").removeClass("disabled")})}}),
"remove"===c.val()&&$.ajax({type:"GET",url:"/admin/products.json?collection_id="+d.val()+"&fields=id,tags",dataType:"json",success:function(a){e.addClass("disabled");u(a,0,f.val(),function(){b.notice("Matched tags have been removed.");e.text("Update tags").removeClass("disabled")})}})):b.notice("Choose a collection",!0);return!1});c.hide();$.ajax({type:"GET",url:"/admin/collections.json?limit=250",dataType:"json",success:function(a){if(a.collections.length){b.data("collections",a);for(var e="",f=
...
a=$("div.metafield-content");b.loadmeta(a,s)}else b.notice("ShopifyFD error : setup_products : Metafield target HTML not found",!0);c.length?$.ajax({type:"GET",url:"/admin/products.json?limit=250&fields=id,title",dataType:"json",success:function(a){if(a){products=a.products;a="";var e=products.length;if(250>=e){for(var d=0;d<e;d++)b.data("omega")!==products[d].id.toString()&&(a+='<option value="'+products[d].id+'">'+products[d].title+"</option>");a=$("<select />",{"class":"header-select fadein"}).append("<option>Choose another product to edit</option>",
a).change(function(){var a=$(this).val();a&&b.redirect("/admin/products/"+a)});a.find("option").sort(b.selectSort).appendTo(a);c.prepend(a)}}},error:function(a){b.notice("Error loading linklist data",!0)}}):b.notice("ShopifyFD error : setup_products : Product switcher target HTML not found",!0);a=$(".section.collections .section-summary");if(a.length){var f=$("#product-custom-collections .remove a");if(f.length){var g=$("<a />",{"class":"btn"}).text("Remove from all").on("click",function(b){b.preventDefault();
f.trigger("click")});a.append(g)}}0===$("#rte_extra").length&&($("#product-description_iframecontainer").eq(0).after('<div id="rte_extra"><a class="btn btn-slim tooltip tooltip-bottom" id="clearformatting" href="#"><span class="tooltip-container"><span class="tooltip-label">Careful, this method is brutal...</span></span>Purge html</a> <a class="btn btn-slim tooltip tooltip-bottom" id="createbackup" href="#"><span class="tooltip-container"><span class="tooltip-label">Save contents as metafield</span></span>Create Backup</a> <a class="btn btn-slim" style="display:none;" id="restorebackup" href="#">Restore Backup</a> <a class="btn btn-slim tooltip tooltip-bottom" id="save_images_to_meta" href="#"><span class="tooltip-container"><span class="tooltip-label">Add image paths to a metafield</span></span>Images to Metafields</a></div>'),
...
setInterval(function(){a.length?a.click():(clearInterval(b.data("autosave")),a=null,b.notice("Autosave disabled"))},3E4)));$(this).toggleClass("rte-command-active");return!1}));b.btn_removealltags();var a=$("h1.header-main"),h={};a.length&&("undefined"!==typeof e&&(d=$("<a />",{href:e,target:"_blank",style:"display: inline-block;vertical-align: top;",title:"View in Store"}),d.append('<i class="next-icon next-icon--20 next-icon--header next-icon--website-slate-lightest"></i>')),a.append(d));d=$(".section.inventory .section-summary").eq(0);
if(d.length){var k=d.find("a.btn"),n=$("<div />",{"class":"box notice hidden fadein",style:"margin-top: 1em;"}),p=$("<a />",{"class":"btn",style:"margin-left:1em"}).text("Bulk Edits");p.on("click",function(b){b.preventDefault();n.toggleClass("hidden")});n.html('<h3>Edit all variants</h3><p style="margin: .5em 0 1em;font-size: 12px;border-bottom: 1px solid #ccc;padding-bottom: .5em;">Bulk Editing comes with risks.</p><label style="margin-top:1.5em">Set Compare at Price<br><small>0 will clear the compare at price</small></label><input type="number" style="width:50%" value="0"> <a class="bulk-compare-save tooltip tooltip-bottom btn btn-slim"><span class="tooltip-container"><span class="tooltip-label">Save Compare at Price for all variants</span></span>Save</a>');
bulkCompareSaveBtn=n.find("a.bulk-compare-save");bulkCompareFields=n.find("input");bulkCompareSaveBtn.on("click",function(a){a.preventDefault();var c=$(this);c.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");a=function(){c.removeClass("is-loading").attr("style","margin-left:1em")};var e=bulkCompareFields.val();if(isNaN(e)||"0"===e||""===e)e=null;if((e||null===e)&&"object"===typeof h)if(1<Object.keys(h).length)if(Object.keys(h).length===$("tr.variant").length){var d=[],
f;for(f in h){var g=h[f],k;for(k in g)g.hasOwnProperty(k)&&d.push({variant:{id:g[k],compare_at_price:e}})}d.length&&b.updateVariant(d,0,a)}else b.notice("Unexpected variant found.",!0);else b.notice("You only have 1 variant. No need to bulk edit",!0)});d.append(n)}else b.notice("ShopifyFD error : setup_products : Inventory target HTML not found",!0);$("#product-inner-variants th:last-child").length?($("#product-inner-variants th:last-child").before("<th>Variant ID</th>"),$("th.inventory.tc").text("#").prop("title",
...
"Edit weight",!0);var a=$('a[data-action="update-variant-weight"]'),c=$('input[data-action="update-variant-weight"]');if(a.length&&c.length)a.on("click",function(){var e=$('tr.variant input[type="checkbox"]:checked');if(e.length&&!isNaN(c.val())){var d=[];e.each(function(){var b=$(this).parent().parent().find("td.vid").text();isNaN(b)||d.push(parseInt(b))});var f=function(e,d){$.ajax({type:"PUT",url:"/admin/variants/"+d[e],dataType:"json",data:{variant:{id:d[0],grams:c.val()}},success:function(c){++e;
e<d.length?f(e,d):(b.notice("Weight updated"),a.removeClass("is-loading").text("Save"))}})};a.addClass("is-loading").text("");f(0,d)}else b.notice("Did you set a weight?",!0);return!1});return!1});$("<span/>",{"class":"hidden"});e.append(v);$('tr.variant input[type="checkbox"]').on("change",function(){1<$('tr.variant input[type="checkbox"]:checked').length?v.removeClass("hidden"):v.addClass("hidden")})}else b.notice("ShopifyFD error : setup_products : unable to add bulk weight editor");d.length&&
"object"===typeof h&&1<Object.keys(h).length&&k.after(p)},setup_pages:function(){var a=$(".section.description .section-summary").eq(0);if(a.length){var c=a.find("a").eq(0).clone(!0),d=c.attr("href");a.after('<div class="sub_section-summary fadein"><h1>Metafields <span id="metacount" class="animated bounce">0</span></h1><div class="metafield-content content"><i class="ico ico-20 ico-20-loading"></i></div></div>').remove();a=$("div.metafield-content");b.loadmeta(a,s)}else b.notice("ShopifyFD error : setup_pages : Metafield target HTML not found",
...
e=a.country.price_based_shipping_rates;f=a.country.carrier_shipping_rate_providers;b.notice("Ready to paste "+c+".");$("a.bulkpaste, input.paste-checkbox").removeClass("hidden")},error:function(){b.notice("Error copying rates",!0)}});return!1}),p=$("<input />",{type:"checkbox",style:"float:right","class":"hidden paste-checkbox"});a=$("<a/>",{href:"#","data-country":b.data("countries").countries[a].name,"data-cid":b.data("countries").countries[a].id,"data-index":a,"class":"hidden fadein bulkpaste btn btn-slim tooltip tooltip-bottom",
style:"float:right;margin-right:.5em"}).html('<span class="tooltip-container"><span class="tooltip-label">Will replace ALL existing rates.</span></span>Paste rates (replace)').on("click",function(){var b=$(this),a=b.attr("data-cid"),c=d,h=e,k=f,n=[],p=0;b.parent().parent().find("tbody").remove();b.parent().find("a").addClass("hidden");for(var q=0,l=c.length;q<l;q++)c[q].country_id=a,delete c[q].id,c[q].type="weight",n[p]=c[q],p++;q=0;for(l=h.length;q<l;q++)h[q].country_id=a,delete h[q].id,h[q].type=
"price",n[p]=h[q],p++;q=0;for(l=k.length;q<l;q++)k[q].country_id=a,delete k[q].id,k[q].type="carrier",n[p]=k[q],p++;g(a,n,b);return!1});c=c.find('a[bind-event-click="addShippingRateModal.show()"]');if(c.length)c.after(n,a,p);else return b.notice("Error. Unable to add copy button.",!0),!1})}else b.notice("Mismatch in country count. Reload this page.",!0)}else b.notice("Loading current country list"),$.ajax({type:"GET",url:"/admin/countries.json",dataType:"json",success:function(a){a.countries.length&&
...
a=$("div.metafield-content"),b.loadmeta(a,s),0===$("#rte_extra").length&&($("#collection-description_iframecontainer").eq(0).after('<div id="rte_extra"><a class="btn btn-slim tooltip tooltip-bottom" id="clearformatting" href="#"><span class="tooltip-container"><span class="tooltip-label">Careful, this method is brutal...</span></span>Purge html</a> <a class="btn btn-slim tooltip tooltip-bottom" id="createbackup" href="#"><span class="tooltip-container"><span class="tooltip-label">Save contents as metafield</span></span>Create Backup</a> <a class="btn btn-slim" style="display:none;" id="restorebackup" href="#">Restore Backup</a> <a class="btn btn-slim tooltip tooltip-bottom" id="save_images_to_meta" href="#"><span class="tooltip-container"><span class="tooltip-label">Add image paths to a metafield</span></span>Images to Metafields</a></div>'),
b.setup_rte())):b.notice("ShopifyFD error : setup_custom_collections : target html not found",!0)},get_files:function(a,c){var d=$("#settings-general table");d.length&&(b.notice("Loading files..."),$.ajax({type:"GET",url:"/admin/files.json?limit=50&direction=next&page="+a,success:function(a,f,g){if(a){$("");f=0;for(g=a.files.length;f<g;f++){var h=$("<tr />",{"class":"file_row"}),k=$("<td />"),n=$("<td />"),p=$("<td />"),l=$("<td />",{"class":"tr"}),s=$("<td />"),w=$("<a />",{href:"#","data-src":a.files[f].public_url.replace("http:",
"")});if(-1<a.files[f].content_type.indexOf("image")){w.on("click",function(){var a=$(this);b.fd_modal(!0,'<img src="'+a.attr("data-src")+'" />',"Image Preview",!0);return!1}).text(a.files[f].key.replace("files/",""));var r=a.files[f].public_url.replace(/(\.[^\.]+)$/,"_thumb$1").replace("http:","");k.html('<img src="'+r+'" />');n.append(w);p.html('<input class="select-all-on-focus" value="'+a.files[f].public_url+'">')}else w.attr("href",a.files[f].public_url).attr("target","_blank").text(a.files[f].key.replace("files/",
""));l.html(Math.floor(a.files[f].size/1024)+" kB");h.append(k,n,p,l,s);d.append(h)}f>=c?(b.notice("All loaded"),$(".header-right .segmented").hide()):(++f,b.get_files(f,c))}}}))},setup_files:function(){var a=$(".header-row .header-right").eq(0);if(a.length){var c=$("<ul/>",{"class":"segmented",id:"get_all_images"}),d=$("<li/>"),e=$("<a/>",{"class":"btn",href:"#",style:"margin-left:1em"}).html("Show all files").on("click",function(){var a=$(this);a.addClass("is-loading").attr("style","margin-left:1em;text-indent: -9999px;");
...
typeof b)return s[a];s[a]=b},toggleStyle:function(){$("html").toggleClass("shopifyJSoverride")},isloading:function(){return b.data("content").hasClass("loading")}}}();b.load_css();b.init()}else alert("Error. This should be run within the Shopify Admin page.")}else alert("Error. jQuery not found.")})();